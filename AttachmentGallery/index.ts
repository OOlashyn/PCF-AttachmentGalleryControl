  ...
  implements ComponentFramework.StandardControl<IInputs, IOutputs>
{
  ...
  public init(
    ...
    this._context.resources.getResource(
      "img/pdf_icon.png",
      this.setPdfImage.bind(this),
      this.showError.bind(this, "ERROR with PDF Image!")
    );
    ...
    notFoundText.innerText =
      "Attachments not found. Press Refresh to try to load them again.";
    ...
    pdfControlsContainer.classList.add(
      "dwc-flex-container",
      "dwc-pageinput-container",
      "dwc-hide"
    );
    ...
    minusIcon.className =
      "header-icon dwc-side-margin-4 ms-Icon ms-Icon--Remove";
    ...
    modalNoteTextContainer.classList.add(
      "dwc-modal-notetext-container",
      "dwc-hide"
    );
    ...
    this.GetAttachments(curentRecord).then((result) =>
      this.CreateGallery(result)
    );
  }
  ...
  private async GetAttachments(
    ...
    const searchQuery =
      "?$select=annotationid,documentbody,mimetype,notetext,subject,filename" +
  ...
  private CreateGallery(result: Attachment[]): any {
    ...
    if (result.length > 0) {
      ...
      for (let i = 0; i < result.length; i++) {
        ...
        newImg.src =
          result[i].mimeType.indexOf("pdf") == -1
            ? this.generateImageSrcUrl(
                result[i].mimeType,
                result[i].documentBody
              )
            : this.pdfImageSrc;
  ...
  private setModalImage(note: Attachment) {
    ...
    if (
      ...
      !this.modalState.isPdfViewerOpen &&
      isAttachmentPdf
    ) {
  ...
  private setPreview(currentNoteNumber: number) {
    if (currentNoteNumber === this._notes.length) {
      currentNoteNumber = 0;
    }
    if (currentNoteNumber < 0) {
      currentNoteNumber = this._notes.length - 1;
    }
    ...
    this._modalHeaderText.innerHTML =
      (currentNoteNumber + 1).toString() +
      " / " +
      this._notes.length.toString() +
      " " +
      currentNote.title;
